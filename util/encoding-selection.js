...
function findEncodingOrder(acceptEncoding, preference, supportedEncodings) {
  if (acceptEncoding) {
    let sortedEncodingList = parseEncoding(acceptEncoding);
    sortedEncodingList = takePreferenceIntoAccount(
      sortedEncodingList,
    ...
    let ret = [];
    for (let ii = 0; ii < sortedEncodingList.length; ++ii) {
      let encoding = sortedEncodingList[ii];
      if (encoding === IDENTITY) {
        return ret;
      }
      if (encoding === '*') {
        for (let jj = 0; jj < supportedEncodings.length; ++jj) {
          let supEnc = supportedEncodings[jj]
          if (!ret.includes(supEnc)) {
            ret.push(supEnc);
          }
        }
...
module.exports = {
  ...
  findEncodingOrder: findEncodingOrder,
};
