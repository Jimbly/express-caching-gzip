...
describe('option', function () {
    it('should handle no options provided', function () {
        ...
        expect(strip(res)).to.deep.equal({ index: 'index.html' });
    });

    it('should parse index options', function () {
        let res = sanitizeOptions({ index: 'main.js' });
        expect(strip(res)).to.deep.equal({ index: 'main.js', serveStatic: { index: 'main.js' } });
    ...
    it('should parse indexFromEmptyFile options', function () {
        ...
        expect(strip(res)).to.deep.equal({ index: 'main.js' });
    ...
    it('should keep serveStatic options', function () {
        ...
        expect(strip(res)).to.deep.equal({ index: 'index.html', serveStatic: { index: 'main.js' } });
    });
    ...
    it('should not overwrite serverStatic.index when already set', function () {
        ...
        expect(strip(res)).to.deep.equal({ index: false, serveStatic: { index: 'index.html' } });
    });

    it('should parse enableBrotli option', function () {
        let res = sanitizeOptions({ enableBrotli: true });
        expect(strip(res)).to.deep.equal({ enableBrotli: true, index: 'index.html' });
    ...
    it('should parse customCompressions', function () {
        ...
        expect(strip(res)).to.deep.equal({ customCompressions: [compression], index: 'index.html' });
    });

    it('should parse empty customCompressions', function () {
        const res = sanitizeOptions({ customCompressions: [] });
        expect(strip(res)).to.deep.equal({ customCompressions: [], index: 'index.html' });
    });
    ...
    it('should parse orderPreference', function () {
        ...
        expect(strip(res)).to.deep.equal({ index: 'index.html', orderPreference: ['br'] });
    });
